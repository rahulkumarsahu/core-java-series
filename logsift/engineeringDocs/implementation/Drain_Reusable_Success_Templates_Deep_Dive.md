# Drain + Reusable Success Templates for Our CI/CD Log Reduction Design

> Beginner-friendly deep dive for our environment: multiple Jenkins instances, many teams/projects/repos, DAG-style pipelines, parallel stages, Jenkins-first log retrieval with UDS fallback, and an existing Spring Boot MCP server.
...
# 45. Can We Put This Logic in Our Existing Spring Boot MCP Server?

Short answer:

> **Technically yes. Architecturally, be careful.**

The MCP server currently sits close to online request/agent interactions.

Offline baseline generation is:

```text
asynchronous
stateful
potentially CPU-heavy
log-volume-heavy
continuous background processing
```

Those characteristics are different from:
...
# 46. Option A — Put Everything Inside the Existing MCP Server
...
flowchart TD
    ...
    MCP --> TOOL["MCP Tools"]
    MCP --> FETCH["Jenkins / UDS Fetch"]
    MCP --> REDUCE["Online Log Reduction"]
    MCP --> WORKER["Offline Baseline Worker"]
    WORKER --> STORE["Baseline Store"]
```
...
```text
fewer deployables
simple first prototype
reuse Jenkins/UDS clients
reuse security/config
```

Disadvantages:

```text
background Drain work competes with MCP requests
large logs can increase memory/CPU pressure
harder to scale independently
template generation failure can affect MCP process
state/background scheduling becomes mixed with request serving
```

---

# 47. Option B — Separate Baseline Worker / Service

```mermaid
flowchart LR
    MCP["Spring Boot MCP Server"]
      --> RED["Log Reduction API"]

    EVENT["Successful Run Event"]
      --> WORKER["Baseline Worker"]

    WORKER --> J["Jenkins / UDS"]
    WORKER --> STORE["Baseline Store"]
...
```text
scale baseline processing independently
...
```text
one more deployable
extra API/event contracts
more operational setup
```

---

# 48. My Recommendation for Our System

Use a middle path.

## Phase 1 / prototype

We can keep the code in the same repository and even the same Spring Boot application if that speeds experimentation.

But separate it logically:
...
Do not execute baseline creation inside the MCP request thread.

Use:

```text
event consumer / background executor
```

and a persistent baseline store.

## Production direction

If load becomes meaningful:

```text
extract baseline worker/reduction processing
into independently scalable service/workload
```

The MCP server should primarily expose tools/orchestrate.

The heavy log preprocessing layer should be independently scalable.

---

# 49. Important Note About Drain3 and Java

The commonly used Drain3 reference implementation is a Python project.

Our MCP server is Spring Boot / Java.

That means we have choices:

```text
A. Implement the Drain algorithm in Java
B. Use a separate Python Drain service/worker
C. Evaluate a compatible maintained implementation for our stack
```

Do not introduce a Python sidecar only because the paper says "Drain".

The architecture should depend on an interface such as:

```java
TemplateExtractionResult extractTemplates(Stream<NormalizedLogLine> lines);
```

not directly on a specific library.

Then the underlying implementation can change later.
...
learn(lines) -> TemplateProfile
match(line, profile) -> TemplateMatch
```
...
getCompatibleBaseline(key, fingerprint)
saveBaseline(profile)
getRecentVersions(key)
```

## Success Run Provider

```text
SuccessRunProvider

getRecentCompatibleSuccessRuns(key, count)
```
...
# 51. Suggested Spring Boot Components

Conceptually:

```text
SuccessRunEventListener
        ↓
SuccessBaselineCoordinator
        ↓
CompatibleRunResolver
        ↓
LogAcquisitionService
        ↓
LogNormalizer
        ↓
SecretRedactor
...
The online side later becomes:

```text
MCP Tool
   ↓
FailureReductionService
   ↓
BaselineRepository
   ↓
TemplateMatcher
...
# 61. Recommended Offline Architecture for Us

```mermaid
flowchart TD
    EVENT["Pipeline SUCCESS Event"]
        --> MAP["Resolve Logical Pipeline Identity"]

    MAP --> FP["Calculate Compatibility Fingerprint"]

    FP --> RUNS["Find Recent Compatible Success Runs"]

    RUNS --> FETCH["Fetch Logs"]

    FETCH --> J{"Jenkins available?"}
    J -->|"Yes"| RAW["Success Logs"]
    J -->|"No"| U["UDS Fallback"]
    U --> RAW
    ...
    M --> DAG["Split / Tag DAG Stages"]

    DAG --> D["Drain Template Mining"]

    D --> PROFILE["Build Success Baseline Profile"]

    PROFILE --> VERSION["Create New Version"]

    VERSION --> STORE["Baseline Store"]

    STORE --> CACHE["Optional Online Cache"]
```

---

# 62. Recommended Online Usage
...
flowchart TD
    FAIL["Pipeline FAILED"]
      --> ID["Resolve Pipeline + Stage Identity"]

    ID --> FP["Calculate Compatibility Fingerprint"]

    FP --> BASE["Load Compatible Success Baseline"]

    FAIL --> LOG["Fetch Failed Log"]
    LOG --> NORMAL["Same Normalize / Redact / Mask Pipeline"]

    NORMAL --> TEMPLATE["Extract Failed-Line Templates"]

    BASE --> DIFF["Log Diff"]
    TEMPLATE --> DIFF

    DIFF --> CAND["Novel / Interesting Candidates"]
```
...
The success baseline and failed log must use compatible:
...
Otherwise comparison is unreliable.
...
MCP:
    ...
    background baseline logic separated from request path

Production scaling:
    independently scalable baseline worker/reduction workload
```
...
- Spring Boot MCP integration strategy;
